...
public class ApiService<T>:IApiService<T>
{
    ...
    private readonly SemaphoreSlim _semaphoreSlim = new SemaphoreSlim(1, 1); // Declare SemaphoreSlim
    ...
    public async Task<T> AddAsync(T entity)
    {
        ...
        {
            _logger.LogInformation($"Adding entity: {jsonData}");
            var response = await _httpClient.PostAsync(_endpoint, content); // Corrected variable name
    ...
    public async Task<bool> UpdateAsync(int id, T entity)
    {
        ...
        {
            _logger.LogInformation($"Updating entity: {jsonData}");
            var response = await _httpClient.PutAsync($"{_endpoint}/{id}", content); // Corrected variable name
            return response.IsSuccessStatusCode;
        }
        finally
        {
    ...
    public async Task<bool> DeleteAsync(int id)
    {
        ...
        try
        {
            var response = await _httpClient.DeleteAsync($"{_endpoint}/{id}"); // Corrected variable name
            return response.IsSuccessStatusCode;
        }
        finally
        {
